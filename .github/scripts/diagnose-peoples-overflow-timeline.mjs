...
try {
  ...
  for (let i = 0; i < 50; i++) {
    ...
    const sample = await cdp.eval(`(() => {
      ...
      const before = {
        ...
        grid: !!grid,
        hologram: !!hologram,
        gridTransform: grid ? getComputedStyle(grid).transform : null,
        hologramOverflowX: hologram ? getComputedStyle(hologram).overflowX : null,
        routeMainTransform: routeMain ? getComputedStyle(routeMain).transform : null,
        routeMainOpacity: routeMain ? getComputedStyle(routeMain).opacity : null,
      };
  ...
  const firstZero = timeline.find(x => x.grid && x.scrollWidth === x.clientWidth && x.forcedX === 0) || null;
  const lastEight = [...timeline].reverse().find(x => x.forcedX > 1) || null;
  const report = { firstZero, lastEight, timeline };
  console.log("PEOPLES_OVERFLOW_TIMELINE", JSON.stringify({ firstZero, lastEight }));
  await fs.mkdir("live-diagnostic-evidence", { recursive: true });
  await fs.writeFile("live-diagnostic-evidence/peoples-overflow-timeline.json", JSON.stringify(report, null, 2));
} finally {
  cdp.close();
}
