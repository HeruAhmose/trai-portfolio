...
${
  STATIC_MODE
    ? "*This public Pages build uses bounded, verified portfolio context and no external model.*"
    : "*Server-connected mode may use an external model when configured.*"
}`;
...
export function HKPortalWidget() {
  ...
  return (
    <div
      role="region"
      aria-label="H.K. TechBridge assistant"
      className="flex flex-col h-full min-h-[400px] max-h-[600px]"
    ...
    >
      ...
      <div className="flex items-center justify-between px-5 py-4 border-b border-[#2ecc71]/15 flex-shrink-0">
        <div className="flex items-center gap-3">
          <motion.div
            ...
            style={{
              background:
                "radial-gradient(circle at 35% 35%, #2ecc71cc, #1a8a4a88)",
              color: "#050709",
            }}
            animate={{
              boxShadow: [
          ...
          <div>
            <p className="text-xs font-mono text-[#2ecc71] tracking-[0.1em]">
              H.K. ASSISTANT
            </p>
            <p className="text-[9px] font-mono text-[#f4f0e6]/30 tracking-[0.08em]">
              {STATIC_MODE
                ? "Verified public context · no external model"
                : "Claude-assisted · server configured"}
            </p>
          </div>
        </div>
        <button
          ...
          aria-label="Reset H.K. conversation"
        >
          <RefreshCw size={13} />
        </button>
      </div>
      ...
      <div className="flex-1 overflow-y-auto px-4 py-4 space-y-3 min-h-0">
            ...
            >
              <div
                ...
                style={{
                  ...
                  color:
                    msg.role === "user" ? "#f4f0e6" : "rgba(244,240,230,0.75)",
                }}
              >
        ...
        {isLoading && (
          ...
          >
            <div
              className="px-4 py-2.5 border border-[#f4f0e6]/06"
              style={{ background: "rgba(244,240,230,0.04)" }}
            >
      ...
      <div className="px-4 pb-4 pt-2 flex-shrink-0 border-t border-[#2ecc71]/10">
        ...
        >
          <input
            ...
            aria-label="Message H.K. about TechBridge"
            disabled={isLoading}
